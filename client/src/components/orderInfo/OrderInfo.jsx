...
import {
  markOrderAsProcessedHandler,
  markOrderAsSentHandler,
} from "../../hooks/useChangeOrderStatus";
  ...
  return (
    <div>
      <div className="logo-logout">
        <Logo2 />
        <Link to={"/"}>
          <button onClick={logoutHandler}>Izloguj se</button>{" "}
        </Link>
      </div>
              ...
              <tr key={order._id}>
                ...
                <td>
                  {order.products.map((product, index) => (
                    <div key={`${product.productId}-${index}`}>
                      Proizvod: {product.productId.preparate} (
                      {product.productId.price} rsd) - Količina:{" "}
                      {product.quantity}
                    </div>
                  ))}
                </td>
                ...
                <td>
                  <OrderStatus
                    ...
                    onProcessedClick={() =>
                      markOrderAsProcessedHandler(order._id)
                    }
